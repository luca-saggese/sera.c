...
// Implementations of the ggml-API stubs declared in q3_ggml_stubs.h plus
...
#include "common.cuh"   // pulls in q3_ggml_stubs.h via redirect headers
...
/* Thread-local stream that q3_naive_pool uses for cudaMallocAsync /
 ...
 * and capture is not invalidated.  Set via q3_pool_set_stream() from
...
static thread_local cudaStream_t t_q3_pool_stream = cudaStreamPerThread;

} // anonymous namespace

/* q3 M1: optional persistent arena for transient kernel scratch (see
 * q3_naive_pool above). Attached once by cuda/q3_mmq.cu. */
unsigned char * q3_arena_base     = nullptr;
size_t          q3_arena_capacity = 0;
size_t          q3_arena_cursor   = 0;

extern "C" void q3_arena_set(unsigned char * base, size_t capacity) {
    q3_arena_base     = base;
    q3_arena_capacity = capacity;
    q3_arena_cursor   = 0;
}

extern "C" void q3_arena_reset(void) {
    q3_arena_cursor = 0;
}

extern "C" void q3_pool_set_stream(cudaStream_t stream) {
    t_q3_pool_stream = stream;
}

extern "C" cudaStream_t q3_pool_get_stream(void) {
    return t_q3_pool_stream;
}

namespace {

struct q3_naive_pool : public ggml_cuda_pool {
    int device;

    explicit q3_naive_pool(int device) : device(device) {}

    void * alloc(size_t size, size_t * actual_size) override {
        /* q3 M1: when the runtime has attached a persistent arena, slice
         * transient scratch out of it so the Q4_K linear hot path performs
         * no cudaMalloc/cudaFree at all. The arena is reset per launcher
         * invocation by the caller. */
        if (q3_arena_base) {
            const size_t aligned = (size + 255u) & ~(size_t)255u;
            if (q3_arena_cursor + aligned > q3_arena_capacity) {
                fprintf(stderr, "q3 pool: workspace exhausted (%zu + %zu > %zu)\n",
                        q3_arena_cursor, aligned, q3_arena_capacity);
                abort();
            }
            void * ptr = q3_arena_base + q3_arena_cursor;
        ...
        void * ptr = nullptr;
        CUDA_CHECK(cudaMallocAsync(&ptr, size, t_q3_pool_stream));
        if (actual_size) *actual_size = size;
        return ptr;
    }
    ...
    void free(void * ptr, size_t size) override {
        if (!ptr) return;
        if (q3_arena_base) return; /* arena-backed: reset by the caller */
        ggml_cuda_set_device(device);
        CUDA_CHECK(cudaFreeAsync(ptr, t_q3_pool_stream));
        (void) size;
    }
};
...
std::unique_ptr<ggml_cuda_pool> ggml_backend_cuda_context::new_pool_for_device(int device, int /*stream_no*/) {
    return std::unique_ptr<ggml_cuda_pool>(new q3_naive_pool(device));
}
